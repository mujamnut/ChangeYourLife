...
Backend AI routing is separated by role. Set `OPENROUTER_API_KEY` and `OPENROUTER_MODEL` for all normal text chat and structured-action planning. Set `LMSTUDIO_BASE_URL=http://127.0.0.1:1234` and `LMSTUDIO_VISION_MODEL=qwen/qwen3.5-9b` only for visual extraction/OCR. A request without visual input skips LM Studio and goes directly to OpenRouter. Direct images and only the scanned/insufficient pages of a PDF are processed by LM Studio; OpenRouter receives sanitized bounded text, never the raw visual payload. Direct images and rendered PDF pages share a four-frame limit, and there is no implicit cross-role fallback. An LM-Studio-only setup reports degraded AI status because it cannot produce the final text/action response, while a setup with neither provider preserves sandbox mode for local compatibility.

`LMSTUDIO_MODEL`, `OPENROUTER_VISION_MODELS`, `OPENROUTER_VISION_MODEL`, and `AI_VISION_MODEL` are still accepted for configuration compatibility but are ignored by the Phase A/B runtime. Routing Phases A and B are backend-only and require no Android/frontend change. See `docs/development.md` and `docs/ai-fallback-audit.md` for the routing, attachment-limit, and timeout policy.
